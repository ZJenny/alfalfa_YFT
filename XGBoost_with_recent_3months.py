    ...
    for date in dates:
        ...
        for plot in window_data["Plot"].unique():
            ...
            def calculate_stats(column):
                return {"min": column.min(), "max": column.max(), "mean": column.mean(), "sum": column.sum()}
...
def train_test_xgboost(data_path, train_experiments, test_experiments, author_name="your_name"):
    ...
    plt.title("Actual vs Predicted Alfalfa Yield (XGBoost Model)")
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(folder_png, f"{author_name}_xgboost_actual_vs_predicted_{timestamp}.png"))
    plt.close()
    ...
    plt.title("Prediction Residuals by Experiment (XGBoost Model)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(folder_png, f"{author_name}_xgboost_residuals_by_experiment_{timestamp}.png"))
    plt.close()
    ...
    plt.barh(range(min(20, len(importance_df))), importance_df['Importance'].values[:20], align='center')
    plt.yticks(range(min(20, len(importance_df))), importance_df['Feature'].values[:20])
    plt.xlabel('Importance')
    plt.title('Top 20 Feature Importance')
    plt.tight_layout()
    plt.savefig(os.path.join(folder_png, f"{author_name}_feature_importance_{timestamp}.png"))
    plt.close()
